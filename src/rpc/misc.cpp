...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total kgs balance of the wallet (excluding zerocoins)\n"
    ...
    UniValue zkgsObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zkgsObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zkgsObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("zKGSsupply", zkgsObj));

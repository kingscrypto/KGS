...
static std::map<int, unsigned int> mapStakeModifierCheckpoints =
    boost::assign::map_list_of(1, 0x889137fbu)
    (50, 0x08f271e0u)
    (100, 0x32738dd3u)
    (250, 0x1b6da2c8u)
    (350, 0x78013f43u);
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZKgsStake(spend));
    } else {
        ...
        CKgsStake* kgsInput = new CKgsStake();
        kgsInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(kgsInput);
    }

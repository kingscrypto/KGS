...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "kgs.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "kgsd.pid"));
#endif
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, kgs, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all KGS specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-kgsstake=<n>", strprintf(_("Enable or disable staking functionality for KGS inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zkgsstake=<n>", strprintf(_("Enable or disable staking functionality for zKGS inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzkgs=<n>", strprintf(_("Enable automatic wallet backups triggered after each zKGS minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zkgsbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zKGS backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizekgsamount=<n>     " + strprintf(_("Keep N KGS anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The KGS Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that KGS is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. KGS Core is shutting down."));
...
#endif
    // Make sure only a single KGS process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. KGS Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //KGS: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //KGS specific: zerocoin and spork DB's
                ...
                // KGS: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // KGS: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of KGS Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart KGS Core to complete") << "\n";
        ...
        zwalletMain = new CzKGSWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zKGSWallet
        uiInterface.InitMessage(_("Syncing zKGS wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnableZKgsBackups = GetBoolArg("-backupzkgs", true);
        pwalletMain->setZKgsAutoBackups(fEnableZKgsBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zkgsTracker->Init();
...
//    }
//
//    nAnonymizeKgsAmount = GetArg("-anonymizekgsamount", 0);
//    if (nAnonymizeKgsAmount > 999999) nAnonymizeKgsAmount = 999999;
//    if (nAnonymizeKgsAmount < 2) nAnonymizeKgsAmount = 2;
    ...
    LogPrintf("Anonymize KGS Amount %d\n", nAnonymizeKgsAmount);

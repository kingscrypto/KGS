...
public:
    ...
    bool WriteZKGSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZKGSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZKGSSeed_deprecated(uint256& seed);
    bool EraseZKGSSeed();
    bool EraseZKGSSeed_deprecated();

    bool WriteZKGSCount(const uint32_t& nCount);
    bool ReadZKGSCount(uint32_t& nCount);

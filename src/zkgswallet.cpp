...
CzKGSWallet::CzKGSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zkgs seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZKGSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZKGSSeed_deprecated()) {
                    LogPrintf("%s: Updated zKGS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zkgs seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zkgs wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzKGSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZKGSCount(nCountLastUsed);
    else if (!walletdb.ReadZKGSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzKGSWallet::Lock()
{
    seedMaster = 0;
}

void CzKGSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzKGSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZKGS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzKGSWallet::LoadMintPoolFromDB()
{
...
void CzKGSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzKGSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzKGSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zkgsTracker->HasPubcoinHash(pMint.first)) {
...
bool CzKGSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZKGS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zkgsTracker which also adds to database
    pwalletMain->zkgsTracker->Add(dMint, true);
...
void CzKGSWallet::SeedToZKGS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzKGSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzKGSWallet::GenerateDeterministicZKGS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzKGSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzKGSWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzKGSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());

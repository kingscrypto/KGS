...
enum ZerocoinSpendStatus {
    ...
    ZKGS_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZKGS_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZKgsBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZKGSOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzkgsAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzKGSTracker> zkgsTracker;
    ...
    void setZWallet(CzKGSWallet* zwallet)
    {
        zwalletMain = zwallet;
        zkgsTracker = std::unique_ptr<CzKGSTracker>(new CzKGSTracker(strWalletFile));
    }

    CzKGSWallet* getZWallet() { return zwalletMain; }
    ...
    void setZKgsAutoBackups(bool fEnabled)
    {
    ...
    /** zKGS reset */
    boost::signals2::signal<void()> NotifyzKGSReset;

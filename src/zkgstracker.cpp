...
CzKGSTracker::CzKGSTracker(std::string strWalletFile)
{
...
CzKGSTracker::~CzKGSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzKGSTracker::Init()
{
...
bool CzKGSTracker::Archive(CMintMeta& meta)
{
...
bool CzKGSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzKGSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzKGSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzKGSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzKGSTracker::GetSerialHashes()
{
...
CAmount CzKGSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzKGSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzKGSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzKGSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzKGSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzKGSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzKGSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzKGSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzKGSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzKGSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzKGSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzKGSWallet* zKGSWallet)
{
    bool iszKGSWalletInitialized = (NULL != zKGSWallet);
    CMintMeta meta;
    ...
    if (! iszKGSWalletInitialized)
        zKGSWallet = new CzKGSWallet(strWalletFile);
    meta.isSeedCorrect = zKGSWallet->CheckSeed(dMint);
    if (! iszKGSWalletInitialized)
        delete zKGSWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzKGSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzKGSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzKGSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzKGSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzKGSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzKGSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzKGSWallet* zKGSWallet = new CzKGSWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zKGSWallet);
        }
        delete zKGSWallet;
        LogPrint("zero", "%s: added %d dzkgs from DB\n", __func__, listDeterministicDB.size());
    }

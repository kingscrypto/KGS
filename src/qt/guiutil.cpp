...
{
    // return if URI is not valid or is no KGS: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert kgs:// to kgs:
    //
    //    Cannot handle this later, because kgs:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open kgs.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "KGS.lnk";
}
...
{
    return GetAutostartDir() / "kgs.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a kgs.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the kgs app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add kgs app to startup item list

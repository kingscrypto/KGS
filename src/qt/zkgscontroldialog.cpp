...
#include "ui_zkgscontroldialog.h"
...
std::set<std::string> ZKgsControlDialog::setSelectedMints;
std::set<CMintMeta> ZKgsControlDialog::setMints;

bool CZKgsControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZKgsControlDialog::COLUMN_DENOMINATION || column == ZKgsControlDialog::COLUMN_VERSION || column == ZKgsControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZKgsControlDialog::ZKgsControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZKgsControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZKgsControlDialog::~ZKgsControlDialog()
{
    delete ui;
}

void ZKgsControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZKgsControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZKgsControlWidgetItem* itemDenom(new CZKgsControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZKgsControlWidgetItem *itemMint = new CZKgsControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "The zKGS seed used to mint this zKGS is not the same as currently hold in the wallet";
            else
...
void ZKgsControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZKgsControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZKgs_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZKgsControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZKgsControlDialog::GetSelectedMints()
{
...
void ZKgsControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

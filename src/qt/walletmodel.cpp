...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered kgs address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal kgs:URI (kgs:XyZ...?message=example)
            {
...
static void NotifyzKGSReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzKGSReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzKGSReset.connect(boost::bind(NotifyzKGSReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzKGSReset.disconnect(boost::bind(NotifyzKGSReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zkgsTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}

...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeKgsAmount"))
        settings.setValue("nAnonymizeKgsAmount", 1000);

    nAnonymizeKgsAmount = settings.value("nAnonymizeKgsAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeKgsAmount"))
        SoftSetArg("-anonymizekgsamount", settings.value("nAnonymizeKgsAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in kgs.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeKgsAmount:
            return QVariant(nAnonymizeKgsAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeKgsAmount:
            nAnonymizeKgsAmount = value.toInt();
            settings.setValue("nAnonymizeKgsAmount", nAnonymizeKgsAmount);
            emit anonymizeKgsAmountChanged(nAnonymizeKgsAmount);
            break;
        case CoinControlFeatures:

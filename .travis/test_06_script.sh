...
cd "kgs-$HOST" || (echo "could not enter distdir kgs-$HOST"; exit 1)
...
if [ "$RUN_BENCH" = "true" ]; then
  BEGIN_FOLD bench
  DOCKER_EXEC LD_LIBRARY_PATH=$TRAVIS_BUILD_DIR/depends/$HOST/lib $OUTDIR/bin/bench_kgs -scaling=0.001
  END_FOLD
fi
